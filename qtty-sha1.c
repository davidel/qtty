...
static void sha1_transform(sha1_int32 state[5], unsigned char const *buffer);



static void sha1_transform(sha1_int32 state[5], unsigned char const *buffer)
{
	sha1_int32 a, b, c, d, e;
	char64long16_t *block;
	unsigned char workspace[64];

	block = (char64long16_t *) workspace;
...
void sha1_final(unsigned char digest[SHA1_DIGEST_SIZE], sha1_ctx_t *context) {
	...
	for (i = 0; i < 8; i++)
		finalcount[i] = (unsigned char)
			((context->count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255);  /* Endian independent */
